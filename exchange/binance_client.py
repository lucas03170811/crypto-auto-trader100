...
class BinanceClient:
    """
    UMFutures 的 async 輕量封裝：
    - klines 用關鍵字參數
    - 新增 get_position / close_position，支援移動停利/停損與加碼滾倉
    - 以 Semaphore 控制併發，避免連線池爆滿
    """
    ...
    async def get_position(self, symbol: str) -> Optional[dict]:
        """
        回傳 dict: {entryPrice, positionAmt, unrealizedProfit, leverage}
        positionAmt > 0 代表多單，< 0 代表空單，=0 無倉位
        """
        try:
